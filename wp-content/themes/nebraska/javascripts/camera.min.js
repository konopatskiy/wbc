...
;(function($){$.fn.camera = function(opts, callback) {
	...
	var defaults = {
		...
		barDirection		: 'topToBottom',	//'leftToRight', 'rightToLeft', 'topToBottom', 'bottomToTop'
...
////////callbacks
	...
	fakeHover.append(
		'<div class="camera_navigation container"></div>'
	)
	...
	if(loader == 'pie'){
		fakeHover.find('.camera_navigation').append(
	...
	if(opts.pagination==true){
		wrap.find('.camera_navigation').append(
	...
	if(elem.length!=0){
		...
		if (loader != 'pie') {
			...
			switch(opts.piePosition){
				case 'leftTop' :
					piePosition = 'left:10px; top:5px;';
					break;
				case 'rightTop' :
					piePosition = 'right:10px; top:5px;';
					break;
				case 'leftBottom' :
					piePosition = 'left:10px; bottom:5px;';
					break;
				case 'rightBottom' :
					piePosition = 'right:10px; bottom:5px;';
					break;
			}
		...
		if($(thumbs).length) {
			...
			if(!$(pagination).length) {
				...
				//pagination.css({top:-$(pagination).outerHeight()});
			}
			...
			if($(thumbs).length && !$(pagination).length) {
				...
				if(firstPos == true) {
					...
					if($(thumbs).length && !$(pagination).lenght) {
						...
						thumbs.css({top:-$(thumbs).outerHeight()});
					}
					thumbnailVisible();
						...
						if (lMoveInContent!=0){
							...
							$('.cameraContent.cameracurrent .moveFromLeft, .cameraContent.cameracurrent .moveFromRight, .cameraContent.cameracurrent .moveFromTop, .cameraContent.cameracurrent .moveFromBottom, .cameraContent.cameracurrent .fadeIn, .cameraContent.cameracurrent .fadeFromLeft, .cameraContent.cameracurrent .fadeFromRight, .cameraContent.cameracurrent .fadeFromTop, .cameraContent.cameracurrent .fadeFromBottom',fakeHover).each(function(){
								...
								if(tClass.indexOf("moveFromLeft") != -1) {
									...
									t.css('visibility','visible').delay((time/lMoveInContent)*(0.1*(ind-1))).animate({'bottom':'50%',opacity:1},(time/lMoveInContent)*0.15,easeMove);
