...
function recognized_font_families() {
  ...
  $customfonts = get_option_tree( 'custom_font', '', false, true, -1 );
  if(isset($customfonts) && is_array($customfonts)) {
	  foreach ($customfonts as $key => $value) {
		$websafefonts[strtolower($value['title'])] = $value['title'];
	  }
  }
  return apply_filters( 'recognized_font_families', $websafefonts );
}
