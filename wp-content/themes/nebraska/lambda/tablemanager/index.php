...
function lambda_table_view() {
    ...
    foreach ($lambda_table_data as $data) { 
      ...
      echo '<tr>
				...
				<td>
					<span class="shortcode">[lambdatable id="'.$data->id.'"]</span>
				</td>				
	   			<td>
       ...
       <tr>
		   ...
		   <td colspan="6">
				...
				<input type="text" id="table_name" class="lambda_input" name="table_name" size="70" placeholder="<?php _e('Table Name', UT_THEME_NAME); ?>" />
	...
	if(!get_option($option)) {
		...
		if($option){
			...
			if($options) {
					...
					$sql = "INSERT INTO " . $table_lambda_tables . " values (0,'".$option."');";
					if ($wpdb->query( $sql )){
