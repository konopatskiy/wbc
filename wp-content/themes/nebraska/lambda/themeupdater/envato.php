...
class Envato_WP_Toolkit {
        ...
        if ( empty( $errors ) && count( $themes ) > 0 ) {
          ...
          foreach( $themes as $theme ) {
            ...
            if ( $stylesheet && $template && $current_stylesheet !== $stylesheet ) {
              ...
              $links[] = '<a href="' . $delete_url . '" class="submitdelete deletion" title="' . esc_attr( sprintf( __( 'Delete &#8220;%s&#8221;', UT_THEME_INITIAL ), $title ) ) . '" ' . $delete_onclick . '>' . __( 'Delete', UT_THEME_INITIAL ) . '</a>';
              ...
              if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
                foreach ( (array) $submenu['themes.php'] as $item ) {
                  ...
                  if ( ! empty( $submenu[$item[2]] ) ) {
                    ...
                    if ( file_exists( ABSPATH . WP_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}" ) || ! empty( $menu_hook ) )
            ...
            /* Tags list */
            if ( $tags ) {
              $content.= '<p>' . __( 'Tags: ', UT_THEME_INITIAL ). join( ', ', $tags ) . '</p>';
            }
  ...
  public function _http_request_args( $r ){
    if ( ((int) ini_get( 'max_execution_time' ) <  EWPT_PLUGIN_MAX_EXECUTION_TIME) && !ini_get('safe_mode') ) {
